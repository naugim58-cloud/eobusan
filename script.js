...
function reserve(i,amount){const f=fish[i];if(!f.qty)return toast('준비된 수량이 모두 예약되었어요.');$('#modal-content').innerHTML=`<div class="draw-result"><div class="big">💳</div><h2>가상 결제</h2><p><b>${f.area} ${f.name}</b><br>결제 금액 <b>${amount.toLocaleString()}원</b></p><label class="payment-option"><input type="radio" name="pay" checked><span><b>카카오페이</b><small>테스트 결제 · 실제 청구되지 않아요</small></span></label><label class="payment-option"><input type="radio" name="pay"><span><b>신용·체크카드</b><small>테스트 결제 · 실제 청구되지 않아요</small></span></label><label class="payment-option"><input type="radio" name="pay"><span><b>계좌이체</b><small>테스트 결제 · 실제 청구되지 않아요</small></span></label><button class="primary" onclick="completePayment(${i})">${amount.toLocaleString()}원 결제하기</button></div>`}
function completePayment(i){const f=fish[i];f.qty--;rescued++;if(!found.includes(f.area+f.name))found.push(f.area+f.name);render();$('#modal-content').innerHTML=`<div class="draw-result"><div class="big">🎉</div><h2>결제가 완료됐어요!</h2><p><b>${f.area} ${f.name}</b> 구조에 참여해주셔서 감사합니다.</p><div class="pickup-card"><b>📍 픽업 위치</b>${f.area} 수산물 직거래장<br>오늘 16:00 ~ 19:00 수령 가능<br><br><b>👨‍🌾 판매자 연락</b>바다어부 ${f.phone||'010-2458-7721'}<div class="contact-actions"><a href="tel:${f.phone||'01024587721'}">📞 전화하기</a><a href="sms:${f.phone||'01024587721'}">💬 문자 보내기</a></div></div><button class="primary" onclick="closeModal()">확인</button></div>`}
